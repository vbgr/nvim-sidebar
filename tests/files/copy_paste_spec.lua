...
t.test("files copy paste creates unique target when file already exists", function()
  t.temp_dir("files-copy-paste-collision", function(root)
    t.reset_plugin()
    t.write_file(path.join(root, "alpha.txt"), "copied")
    ...
    t.assert_equal(t.read_file(path.join(root, "target", "alpha.txt copy")), "copied")
  end)
end)
...
t.test("files copy paste creates unique target when directory already exists", function()
  t.temp_dir("files-copy-paste-dir-collision", function(root)
    t.reset_plugin()
    t.write_file(path.join(root, "dir-src", "child.txt"), "child")
    t.write_file(path.join(root, "target", "dir-src", "existing.txt"), "existing")
    ...
    t.assert_file_exists(path.join(root, "target", "dir-src", "existing.txt"))
    t.assert_file_exists(path.join(root, "target", "dir-src copy", "child.txt"))
  end)
end)
