...
t.test("files expand and collapse empty directory", function()
  t.temp_dir("files-toggle-empty-directory", function(root)
    t.reset_plugin()
    vim.fn.mkdir(path.join(root, "empty"), "p")

    sidebar.open("files")
    files.actions.open(t.item_by_name("empty"), {
      refresh = sidebar.refresh,
    })

    t.assert_true(expand.is_expanded(path.join(root, "empty")))
    t.assert_equal(t.item_names(), "empty")

    files.actions.collapse(t.item_by_name("empty"), {
      refresh = sidebar.refresh,
    })

    t.assert_false(expand.is_expanded(path.join(root, "empty")))
    t.assert_equal(state.get_current_item().name, "empty")
  end)
end)
